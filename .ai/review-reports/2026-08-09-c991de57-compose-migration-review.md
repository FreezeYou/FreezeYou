...
| **Verdict** | **Not merge-ready** — high regression risk (runtime + UX parity) |
...
- Shared helpers (`AppListItem`, `PackageListScreen`, `EqualButtons`) reduce duplication if themed properly later.
- Diagnosis crash from duplicate LazyColumn keys is avoided (by omitting keys)—data uniqueness still wrong.

---

## Suggested fix order

1. **Crash / security of core flows:** dialog always-allow lifecycle; API-23 locales; SelectShortcutIcon null bitmap.
2. **Main UX parity:** restore FAB anchored `PopupMenu` (+ long-press alpha if desired); fix menu open path.
3. **Main list architecture:** drop ComposeView-in-ListView; pure Compose list **or** classic ImageView adapter; stop per-bind bitmap thrash.
4. **Fragment hosting:** XML `FragmentContainerView` shells for Main / FirstTimeSetup / STAA—not transactions in Compose factory/update.
5. **Theme parity:** map AppCompat theme colors into `FreezeYouTheme` (surface, background, onSurface, primary).
6. **Per-screen UX parity pass:** row typography (18/12), status-dot size (10dp), no unexpected dividers where lists were divider-less, STMA selection chrome, logcat read-only + scroll-to-end, LSCAGA non-editable pickers, app lock borderless button.
7. **Diagnosis unique IDs + keys.**
